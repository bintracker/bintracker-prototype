...
void Main_Window::handle_mouse_input() {
    ...
    if (~mouseState.buttons & 1) {
        ...
        if (status.panelButtonHighlighted >= 0) {
            ...
            case 1:
                al_draw_tinted_bitmap(arrowRight, settings.sysColor,
                                      blockPanel.printArea.topLeft.x + SMALL_BUTTON_SIZE() + 12 * BT_CHAR_WIDTH()
                                      + 3 * DEFAULT_MARGIN(),
                                      settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                break;
            case 2:
                al_draw_tinted_bitmap(buttonPlus, settings.sysColor,
                                      blockPanel.printArea.topLeft.x + 2 * SMALL_BUTTON_SIZE() + 22 * BT_CHAR_WIDTH()
                                      + 5 * DEFAULT_MARGIN(),
                                      settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                break;
            case 3:
                al_draw_tinted_bitmap(buttonMinus, settings.sysColor,
                                      blockPanel.printArea.topLeft.x + 3 * SMALL_BUTTON_SIZE() + 22 * BT_CHAR_WIDTH()
                                      + 6 * DEFAULT_MARGIN(),
                                      settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                break;
            case 4:
                al_draw_tinted_bitmap(buttonInsert, settings.sysColor,
                                      blockPanel.printArea.topLeft.x + 4 * SMALL_BUTTON_SIZE() + 22 * BT_CHAR_WIDTH()
                                      + 7 * DEFAULT_MARGIN(),
                                      settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                break;
            case 5:
                al_draw_tinted_bitmap(buttonRemove, settings.sysColor,
                                      blockPanel.printArea.topLeft.x + 5 * SMALL_BUTTON_SIZE() + 22 * BT_CHAR_WIDTH()
                                      + 8 * DEFAULT_MARGIN(),
                                      settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                break;
            case 6:
                al_draw_tinted_bitmap(buttonExpand, settings.sysColor,
                                      blockPanel.printArea.topLeft.x + 6 * SMALL_BUTTON_SIZE() + 24 * BT_CHAR_WIDTH()
                                      + 10 * DEFAULT_MARGIN(),
                                      settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                break;
            case 7:
                al_draw_tinted_bitmap(buttonShrink, settings.sysColor,
                                      blockPanel.printArea.topLeft.x + 7 * SMALL_BUTTON_SIZE() + 24 * BT_CHAR_WIDTH()
                                      + 11 * DEFAULT_MARGIN(),
                                      settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                break;
            case 10:
            ...
            case 11: {
                if (status.get_current_blocktype() == 0) {
                    al_draw_tinted_bitmap(buttonInsert, settings.sysColor,
                                          referencePanel.printArea.topLeft.x + 1 * SMALL_BUTTON_SIZE()
                                          + 1 * DEFAULT_MARGIN(),
                                          settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                } else {
                    al_draw_tinted_bitmap(buttonRemove, settings.sysColor,
            ...
            case 12: {
                if (status.get_current_blocktype() == 0) {
                    al_draw_tinted_bitmap(buttonRemove, settings.sysColor,
                                          referencePanel.printArea.topLeft.x + 2 * SMALL_BUTTON_SIZE()
                                          + 2 * DEFAULT_MARGIN(),
                                          settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                } else {
            ...
            case 13:
                al_draw_tinted_bitmap(buttonClone, settings.sysColor,
                                      referencePanel.printArea.topLeft.x + 3 * SMALL_BUTTON_SIZE()
                                      + 3 * DEFAULT_MARGIN(),
                                      settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                break;
            case 14:
                al_draw_tinted_bitmap(buttonLooppoint, settings.sysColor,
                                      referencePanel.printArea.topLeft.x + 4 * SMALL_BUTTON_SIZE()
                                      + 4 * DEFAULT_MARGIN(),
                                      settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                break;
            case 20:
                al_draw_tinted_bitmap(arrowDown, settings.sysColor,
                                      2 * DEFAULT_MARGIN() + ((BUTTON_SIZE() + 2 * DEFAULT_MARGIN())
                                              * menuButtons.size()) + (10 * BT_CHAR_WIDTH()),
        ...
        if (mouseState.y < static_cast<int>(BUTTON_PANEL_HEIGHT())) {
            if (mouseState.y <= 0) {
                // workaround for buttons lighting up if mouse is initially out of display bounds
            } else if (mouseState.x < 2 + MENU_BUTTON_SIZE) {
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "New...");
                ...
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Open... (Ctrl+O)");
                al_draw_tinted_bitmap(menuButtons[1],
                                      settings.rowActColor, BUTTON_SIZE() + 3.0f * DEFAULT_MARGIN(), 6.0f, 0);
                status.menuButtonHighlighted = 1;
            } else if (mouseState.x < 2 + MENU_BUTTON_SIZE * 3) {
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Save (Ctrl+S)");
                al_draw_tinted_bitmap(menuButtons[2],
                                      settings.rowActColor, 2.0f * BUTTON_SIZE() + 5.0f * DEFAULT_MARGIN(), 6.0f, 0);
                status.menuButtonHighlighted = 2;
            } else if (mouseState.x < 2 + MENU_BUTTON_SIZE * 4) {
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Save As... (Ctrl+Shift+S)");
                al_draw_tinted_bitmap(menuButtons[3],
                                      settings.rowActColor, 3.0f * BUTTON_SIZE() + 7.0f * DEFAULT_MARGIN(), 6.0f, 0);
                status.menuButtonHighlighted = 3;
            } else if (mouseState.x < 2 + MENU_BUTTON_SIZE * 5) {
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Play from Start (F5)");
                al_draw_tinted_bitmap(menuButtons[4],
                                      settings.rowActColor, 4.0f * BUTTON_SIZE() + 9.0f * DEFAULT_MARGIN(), 6.0f, 0);
                status.menuButtonHighlighted = 4;
            } else if (mouseState.x < 2 + MENU_BUTTON_SIZE * 6) {
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Play from Current Position (F6)");
                al_draw_tinted_bitmap(menuButtons[5],
                                      settings.rowActColor, 5.0f * BUTTON_SIZE() + 11.0f * DEFAULT_MARGIN(), 6.0f, 0);
                status.menuButtonHighlighted = 5;
            } else if (mouseState.x < 2 + MENU_BUTTON_SIZE * 7) {
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Play Pattern (F7)");
                al_draw_tinted_bitmap(menuButtons[6],
                                      settings.rowActColor, 6.0f * BUTTON_SIZE() + 13.0f * DEFAULT_MARGIN(), 6.0f, 0);
                status.menuButtonHighlighted = 6;
            } else if (mouseState.x < 2 + MENU_BUTTON_SIZE * 8) {
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Stop (F8)");
                al_draw_tinted_bitmap(menuButtons[7],
                                      settings.rowActColor, 7.0f * BUTTON_SIZE() + 15.0f * DEFAULT_MARGIN(), 6.0f, 0);
                status.menuButtonHighlighted = 7;
            } else if (mouseState.x < 2 + MENU_BUTTON_SIZE * 9) {
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Undo (Ctrl+Z)");
                al_draw_tinted_bitmap(menuButtons[8],
                                      settings.rowActColor, 8.0f * BUTTON_SIZE() + 17.0f * DEFAULT_MARGIN(), 6.0f, 0);
                status.menuButtonHighlighted = 8;
            } else if (mouseState.x < 2 + MENU_BUTTON_SIZE * 10) {
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Redo (Ctrl+Y)");
                al_draw_tinted_bitmap(menuButtons[9],
                                      settings.rowActColor, 9.0f * BUTTON_SIZE() + 19.0f * DEFAULT_MARGIN(), 6.0f, 0);
                status.menuButtonHighlighted = 9;
            } else if (mouseState.x < 2 + MENU_BUTTON_SIZE * 11) {
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Copy (Ctrl+C)");
                al_draw_tinted_bitmap(menuButtons[10],
                                      settings.rowActColor, 10.0f * BUTTON_SIZE() + 21.0f * DEFAULT_MARGIN(), 6.0f, 0);
                status.menuButtonHighlighted = 10;
            } else if (mouseState.x < 2 + MENU_BUTTON_SIZE * 12) {
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Cut (Ctrl+X)");
                al_draw_tinted_bitmap(menuButtons[11],
                                      settings.rowActColor, 11.0f * BUTTON_SIZE() + 23.0f * DEFAULT_MARGIN(), 6.0f, 0);
                status.menuButtonHighlighted = 11;
            } else if (mouseState.x < 2 + MENU_BUTTON_SIZE * 13) {
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Delete (Cut & Shift)(Ctrl+Del)");
                al_draw_tinted_bitmap(menuButtons[12],
                                      settings.rowActColor, 12.0f * BUTTON_SIZE() + 25.0f * DEFAULT_MARGIN(), 6.0f, 0);
                status.menuButtonHighlighted = 12;
            } else if (mouseState.x < 2 + MENU_BUTTON_SIZE * 14) {
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Paste (Ctrl+P)");
                al_draw_tinted_bitmap(menuButtons[13],
                                      settings.rowActColor, 13.0f * BUTTON_SIZE() + 27.0f * DEFAULT_MARGIN(), 6.0f, 0);
                status.menuButtonHighlighted = 13;
            } else if (mouseState.x < 2 + MENU_BUTTON_SIZE * 15) {
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Insert (Ctrl+V)");
                al_draw_tinted_bitmap(menuButtons[14],
                                      settings.rowActColor, 14.0f * BUTTON_SIZE() + 29.0f * DEFAULT_MARGIN(), 6.0f, 0);
                ...
                if (mouseState.y < static_cast<int>(buttonPanel.printArea.topLeft.y + SMALL_BUTTON_SIZE() + 1.0f)) {
                    if (mouseState.x < 2 * DEFAULT_MARGIN()
                            + ((BUTTON_SIZE() + 2 * DEFAULT_MARGIN()) * menuButtons.size())
                            + (10 * BT_CHAR_WIDTH()) + SMALL_BUTTON_SIZE() + 2.0f) {
                        al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                                     messagePanel.printArea.topLeft.y,
                    ...
                    if (mouseState.x < 2 * DEFAULT_MARGIN() + ((BUTTON_SIZE()
                            + 2 * DEFAULT_MARGIN()) * menuButtons.size())
                            + (10 * BT_CHAR_WIDTH()) + SMALL_BUTTON_SIZE() + 2.0f) {
                        al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                                     messagePanel.printArea.topLeft.y,
                ...
                status.menuButtonHighlighted = -1;
                   ...
                   && status.currentTab != 0) {       // block/reflist panel
            status.panelButtonHighlighted = -1;
            ...
            if (mouseState.x < static_cast<int>(settings.blockNameArea.topLeft.x)) {
                ...
                al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                             messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Rename Block (Alt+R)");
                ...
                if (mouseState.x < static_cast<int>(settings.blockLengthArea.bottomRight.x
                    ...
                    al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                                 messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Add Row (Alt++)");
                    al_draw_tinted_bitmap(buttonPlus, settings.rowActColor,
                                          blockPanel.printArea.topLeft.x + 2 * SMALL_BUTTON_SIZE()
                                          + 22 * BT_CHAR_WIDTH() + 5 * DEFAULT_MARGIN(),
                                          settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                    status.panelButtonHighlighted = 2;
                } else if (mouseState.x < static_cast<int>(settings.blockLengthArea.bottomRight.x
                           + DEFAULT_MARGIN()) + SMALL_BUTTON_WIDTH * 2) {
                    al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                                 messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Remove Last Row (Alt+-)");
                    al_draw_tinted_bitmap(buttonMinus, settings.rowActColor,
                                          blockPanel.printArea.topLeft.x + 3 * SMALL_BUTTON_SIZE()
                                          + 22 * BT_CHAR_WIDTH() + 6 * DEFAULT_MARGIN(),
                                          settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                    status.panelButtonHighlighted = 3;
                } else if (mouseState.x < static_cast<int>(settings.blockLengthArea.bottomRight.x
                           + DEFAULT_MARGIN()) + SMALL_BUTTON_WIDTH * 3) {
                    al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                                 messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Insert Row (Alt+V)");
                    al_draw_tinted_bitmap(buttonInsert, settings.rowActColor,
                                          blockPanel.printArea.topLeft.x + 4 * SMALL_BUTTON_SIZE()
                                          + 22 * BT_CHAR_WIDTH() + 7 * DEFAULT_MARGIN(),
                                          settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                    status.panelButtonHighlighted = 4;
                } else if (mouseState.x < static_cast<int>(settings.blockLengthArea.bottomRight.x
                           + DEFAULT_MARGIN()) + SMALL_BUTTON_WIDTH * 4) {
                    al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                                 messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Delete Row (Alt+Del)");
                    al_draw_tinted_bitmap(buttonRemove, settings.rowActColor,
                                          blockPanel.printArea.topLeft.x + 5 * SMALL_BUTTON_SIZE()
                                          + 22 * BT_CHAR_WIDTH() + 8 * DEFAULT_MARGIN(),
                                          settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                    status.panelButtonHighlighted = 5;
                } else if (mouseState.x < static_cast<int>(settings.blockLengthArea.bottomRight.x
                           + DEFAULT_MARGIN()) + SMALL_BUTTON_WIDTH * 5 + 24) {
                    // TODO(utz): temporary hard-coded offset for expand factor area
                    al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                                 messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Expand (Alt+X)");
                    al_draw_tinted_bitmap(buttonExpand, settings.rowActColor,
                                          blockPanel.printArea.topLeft.x + 6 * SMALL_BUTTON_SIZE()
                                          + 24 * BT_CHAR_WIDTH() + 10 * DEFAULT_MARGIN(),
                                          settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                    ...
                    // TODO(utz): temporary hard-coded offset for expand factor area
                    al_draw_text(font, settings.sysColor, messagePanel.printArea.topLeft.x,
                                 messagePanel.printArea.topLeft.y, ALLEGRO_ALIGN_LEFT, "Shrink (Alt+Shift+X)");
                    al_draw_tinted_bitmap(buttonShrink, settings.rowActColor,
                                          blockPanel.printArea.topLeft.x + 7 * SMALL_BUTTON_SIZE()
                                          + 24 * BT_CHAR_WIDTH() + 11 * DEFAULT_MARGIN(),
                                          settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
                ...
                if (status.get_current_blocktype() == 0) {
                    ...
                    if (mouseState.x < static_cast<int>(settings.referencePanelArea.topLeft.x + DEFAULT_MARGIN())
                        ...
                        al_draw_tinted_bitmap(buttonInsert, settings.rowActColor,
                                              referencePanel.printArea.topLeft.x
                                              + 1 * SMALL_BUTTON_SIZE() + 1 * DEFAULT_MARGIN(),
                                              settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
    ...
    if (mouseState.y < static_cast<int>(BUTTON_PANEL_HEIGHT())) {
        if (!status.leftMouseButtonDown) {
            ...
            status.leftMouseButtonDown = true;
                                         ...
                                         && mouseState.x
                                         >= static_cast<int>(blockPanel.totalArea.bottomRight.x - SCROLL_PANE_SIZE())
        ...
        if (mouseState.y <= static_cast<int>(BUTTON_PANEL_HEIGHT() + SMALL_BUTTON_SIZE())) {
            // top arrow button
            scroll_up();
        } else if (mouseState.y < static_cast<int>(blockPanel.totalArea.bottomRight.y - SMALL_BUTTON_SIZE())) {
            // scroll pane
            if (status.leftMouseButtonDown) {
                scroll_drag_v(mouseState.y);
            } else {
            ...
            scroll_down();
                                            ...
                                            && mouseState.x
                                            >= static_cast<int>(referencePanel.totalArea.bottomRight.x
                                                    - SCROLL_PANE_SIZE()))) {
    ...
    } else if (status.mouseDragLockH || (mouseState.y >= static_cast<int>(blockPanel.totalArea.bottomRight.y
                                         - SCROLL_PANE_SIZE())
        ...
        if (!status.leftMouseButtonDown) {
            ...
            if (status.currentTab == 0) {
                ...
                // TODO(utz): first check is redundant?
                while (col < status.get_current_block_pointer()->columns.size()
                        && col < status.get_visible_last_column()
        ...
        if (!status.leftMouseButtonDown) {
            ...
            // TODO(utz): scroll block area when status.focusBlock
            // TODO(utz): scroll down if mouseState.y > referenceDataArea.bottomRight.y
