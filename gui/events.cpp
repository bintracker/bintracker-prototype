...
void Main_Window::handle_event(const ALLEGRO_EVENT &ev) {
    switch (ev.type) {
...
void Main_Window::play_from_start() {   // TODO(utz): fixed asm address, really???
...
void Main_Window::play_from_current_pos() {
    ...
    bool verbose = false;
    // TODO(utz): actually, we should derive the bock from sequence position when !focusBlock
    mdModule mdf(currentTune.generate_lines_from_pos(status.get_current_block(),
                 ((status.focusBlock && status.currentTab != 0) ? status.get_current_cursor_row(): 0),
                 status.get_current_reference_row()), currentTune.config, verbose);
...
void Main_Window::play_pattern() {  // TODO(utz): fixed asm address, really???
    // TODO(utz): this will give problems when currentTab != 1;
    currentTune.generate_pattern_lines(status.get_current_block());
    bool verbose = false;
