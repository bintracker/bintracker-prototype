...
void Display_Status::init(Work_Tune *currentTune, const Global_Settings &settings) {
    ...
    // TODO(utz): if note range max < 5th octave change currentOctave to note_range_max_oct - 1
    reset();
}
...
void Display_Status::reset() {
    ...
    for (unsigned i = 0; i < workTune->blockTypes.size(); i++) {
        if (currentTab == 1) {
            currentBlocks.push_back(get_reference_iterator(workTune->sequence[0]));
        } else {
...
// TODO(utz): also handle things if focusBlock = true
void Display_Status::set_current_block_from_cursor() {
    if (focusBlock) {
        ...
        if (bt == 0) {
            currentBlocks[bt] = get_reference_iterator(workTune->sequence[get_current_reference_row()]);
        } else {
            currentBlocks[bt] = get_reference_iterator(workTune
                                ->blockTypes[bt].blocks[get_current_reference_row()].name);
        }
...
void Display_Status::inc_current_cursor_column() {
    ...
    if (currentCsrColumns[get_current_blocktype()] < visibleLastColumns[get_current_blocktype()]) {
        ...
        if (visibleLastColumns[get_current_blocktype()] < (get_current_block_pointer()->columns.size() - 1)) {
            ...
            calculate_visible_last_column();
            if (visibleLastColumns[get_current_blocktype()] == prevLastColumn)
                visibleFirstColumns[get_current_blocktype()]++;
            ...
            currentCsrColumns[get_current_blocktype()] = 0;
            // TODO(utz): delete old highlight if all columns visible)
        }
    }
}
...
void Display_Status::calculate_visible_last_column() {
    ...
    // TODO(utz): a while loop might be a better idea here
...
void Display_Status::calculate_visible_first_column() {
    ...
    for (visibleFirstColumns[bt] = visibleLastColumns[bt],
            ...
            // TODO(utz): this might potentially cause trouble, but "visibleFirstColumn >=" is always true
            xpos >= globalSettings->columnHeaderArea.topLeft.x;
...
void Display_Status::inc_current_cursor_row(unsigned amount) {
    if (currentTab == 0) {
        // TODO(utz): handle cases when not all globals fit on screen
    ...
    if (visibleRowsMax < blockLength) {
        if (currentCsrRows[get_current_blocktype()]
                < (visibleFirstRows[get_current_blocktype()] + visibleRowsMax - amount)) {
            ...
            if (visibleRowsMax < currentCsrRows[get_current_blocktype()])
                visibleFirstRows[get_current_blocktype()]
                    = (currentCsrRows[get_current_blocktype()] - visibleRowsMax) / 2;
        }
    } else {
        if (currentCsrRows[get_current_blocktype()] + amount < blockLength) {
            currentCsrRows[get_current_blocktype()] += amount;
        } else {
            if (amount < blockLength) {
                currentCsrRows[get_current_blocktype()]
                    = amount - (blockLength - currentCsrRows[get_current_blocktype()]);
            } else {
                currentCsrRows[get_current_blocktype()] = amount + currentCsrRows[get_current_blocktype()];
                while (currentCsrRows[get_current_blocktype()] >= blockLength) {
                    currentCsrRows[get_current_blocktype()] -= blockLength;
                }
            }
        }
    }
...
void Display_Status::dec_current_cursor_row(unsigned amount) {
    ...
    if (visibleRowsMax < blockLength) {
        if (currentCsrRows[bt] >= visibleFirstRows[bt] + amount) {
    ...
    if (currentTab == 1) {
        if (visibleReferenceRowsMax < workTune->sequence.size()) {
            if (amount < workTune->sequence.size()) {
                if (get_current_reference_row() + amount
                        < (get_visible_first_reference_row() + visibleReferenceRowsMax)) {
                    ...
                    visibleFirstReferenceRows[get_current_blocktype()] =
                        (currentReferenceRows[0] >= visibleReferenceRowsMax)
            ...
            if (amount < workTune->blockTypes[get_current_blocktype()].blocks.size()) {
                if (get_current_reference_row() + amount
                    ...
                    currentReferenceRows[get_current_blocktype()] += amount;
                           ...
                           < (workTune->blockTypes[get_current_blocktype()].blocks.size())) {
                    currentReferenceRows[get_current_blocktype()] += amount;
                    visibleFirstReferenceRows[get_current_blocktype()]
        ...
        if (amount <= workTune->sequence.size()) {
            if (visibleReferenceRowsMax <= workTune->sequence.size()) {
                if (get_current_reference_row() >= get_visible_first_reference_row() + amount) {
                    ...
                    visibleFirstReferenceRows[get_current_blocktype()]
                        = workTune->sequence.size() - visibleReferenceRowsMax;
                }
            } else {
                if (get_current_reference_row() >= amount) {
                    currentReferenceRows[get_current_blocktype()] -= amount;
        ...
        if (amount < workTune->blockTypes[get_current_blocktype()].blocks.size()) {
            if (visibleReferenceRowsMax <= workTune->blockTypes[get_current_blocktype()].blocks.size()) {
                if (get_current_reference_row() >= get_visible_first_reference_row() + amount) {
                    ...
                    currentReferenceRows[get_current_blocktype()]
                        = workTune->blockTypes[get_current_blocktype()].blocks.size()
                          - (amount - get_current_reference_row());
                    visibleFirstReferenceRows[get_current_blocktype()]
...
void Display_Status::set_cursor_to_end() {
    if (focusBlock) {
        if (currentTab == 0) {
            currentGlobal = workTune->globalConstants.size() - 1;
        } else {
        ...
        if (get_current_blocktype() == 0) {
            currentReferenceRows[0] = workTune->sequence.size() - 1;
        } else {
            currentReferenceRows[get_current_blocktype()]
...
bool Display_Status::is_selected(const unsigned &row, const unsigned &col) {
    ...
    if (focusBlock) {
        ...
        if (row >= selectionRowFirst && row <= selectionRowLast
                && col >= selectionColumnFirst && col <= selectionColumnLast)
            return true;
    } else  {
...
void Display_Status::update_selection() {
    ...
    if (selectionType == BLOCK) {
        ...
        if (selectionDirRight) {
            if (nextCol >= selectionColumnFirst) {
            ...
            if (nextCol <= selectionColumnLast) {
                selectionColumnFirst = nextCol;
            } else {
                selectionDirRight = true;
