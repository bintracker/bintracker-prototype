...
Main_Window::Main_Window(const string &initFile): userInputString("") {
    ...
    for (unsigned i = 0; i < settings.configList.size(); i++) {
        al_append_menu_item(configListMenu, settings.configList[i].c_str(),
                            (MENU_FILE_NEW_CONFIGSEL + i), 0, nullptr, nullptr);
    }
    ...
    al_append_menu_item(editMenu, "Inverse Porous Paste (Ctrl+Alt+P)", MENU_EDIT_INVERSEPOROUSPASTE,
                        0, nullptr, nullptr);
...
void Main_Window::init_track_view() {
    ...
    al_draw_tinted_bitmap(arrowDown, settings.sysColor, xpos,
    ...
    al_draw_tinted_bitmap(arrowUp, settings.sysColor, xpos,
                          buttonPanel.printArea.topLeft.y + SMALL_BUTTON_SIZE() + 1.0f, 0);
...
void Main_Window::print_base_octave() {
    float xpos = DEFAULT_MARGIN() + ((BUTTON_SIZE() + 2 * DEFAULT_MARGIN()) * menuButtons.size())
                 + (9 * BT_CHAR_WIDTH());
...
void Main_Window::print_autoinc_setting() {
    ...
    al_draw_filled_rectangle(xpos, buttonPanel.printArea.bottomRight.y - CHAR_HEIGHT() + 2.0f,
                             xpos + (2 * BT_CHAR_WIDTH()), buttonPanel.printArea.bottomRight.y, settings.bgColor);
...
void Main_Window::print_block_tabs() {
    ...
    for (size_t i = 0; i < tabPanel.size(); i++) {
        ...
        if (i == status.currentTab) al_draw_line(tabPanel[i].xpos, settings.tabArea.bottomRight.y,
                    tabPanel[i].xpos + tabPanel[i].xsize, settings.tabArea.bottomRight.y, settings.bgColor,
                    THICK_LINE());
    }
...
void Main_Window::print_block() {
    ...
    // TOD(utz): fields for block length and expand/shrink param
    al_draw_text(font, settings.sysColor, xpos, settings.tabArea.bottomRight.y + DEFAULT_MARGIN() + 2.0f,
                 ALLEGRO_ALIGN_LEFT, "LENGTH");
    xpos += ((10 * BT_CHAR_WIDTH()) + DEFAULT_MARGIN());
    ...
    xpos += (SMALL_BUTTON_SIZE() + DEFAULT_MARGIN());
    al_draw_text(font, settings.sysColor, xpos, settings.tabArea.bottomRight.y + DEFAULT_MARGIN() + 2.0f,
                 ALLEGRO_ALIGN_LEFT, "x");
    xpos += BT_CHAR_WIDTH();
    al_draw_text(font, settings.rowHlColor, xpos, settings.tabArea.bottomRight.y + DEFAULT_MARGIN() + 2.0f,
                 ALLEGRO_ALIGN_LEFT, numToStr(status.expandFactor, 0, settings.hexMode).data());
    ...
    al_draw_text(font, settings.sysColor, settings.rowNumberArea.topLeft.x, settings.rowNumberArea.topLeft.y,
                 ALLEGRO_ALIGN_LEFT, "ROW");
...
void Main_Window::print_globals() {
    // TODO(utz) this is the only occurance of "blockArea" outside of settings -> remove and make private
    settings.blockArea.clear(settings.bgColor);
    float ypos = settings.blockDataArea.topLeft.y;
    unsigned pos = 0;

    for (auto&& it : currentTune.globalConstants) {
        al_draw_text(font, settings.sysColor, settings.blockDataArea.topLeft.x, ypos, ALLEGRO_ALIGN_LEFT,
                     it.command->mdCmdName.data());
        if (pos == status.currentGlobal) {
            al_draw_filled_rectangle(settings.blockDataArea.topLeft.x + (12 * BT_CHAR_WIDTH()), ypos,
                                     settings.blockDataArea.topLeft.x + ((12 + it.printString.size())
                                             * BT_CHAR_WIDTH()), ypos + CHAR_HEIGHT(), settings.cursorColor);
        }
        al_draw_text(font, (pos == status.currentGlobal) ? settings.rowActColor : settings.rowColor,
                     settings.blockDataArea.topLeft.x + (12 * BT_CHAR_WIDTH()), ypos, ALLEGRO_ALIGN_LEFT,
                     it.printString.data());
        ypos += CHAR_HEIGHT();
        pos++;
    }
...
void Main_Window::print_stats() {
    ...
    ypos += CHAR_HEIGHT();
    al_draw_text(font, settings.rowHlColor, xpos, ypos, ALLEGRO_ALIGN_LEFT,
                 numToStr(currentTune.musicdataBinary.binLength + currentTune.engineSizeNoLoop, 0,
...
void Main_Window::print_reference_panel() {
    // TODO(utz): blockHeader ypos is calculated from tabPanel, but referenceHeader x is calculated as
    // referencePanel.printArea.topLeft.x + 3 * BT_CHAR_WIDTH()
    referencePanel.printArea.clear(settings.bgColor);
    ...
    if (status.currentTab == 1) {
        ...
        al_draw_tinted_bitmap(buttonPlus, settings.sysColor, xpos,
                              settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
        xpos += (SMALL_BUTTON_SIZE() + DEFAULT_MARGIN());
        al_draw_tinted_bitmap(buttonInsert, settings.sysColor, xpos,
                              settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
        xpos += (SMALL_BUTTON_SIZE() + DEFAULT_MARGIN());
        al_draw_tinted_bitmap(buttonRemove, settings.sysColor, xpos,
                              settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
        xpos += (SMALL_BUTTON_SIZE() + DEFAULT_MARGIN());
        al_draw_tinted_bitmap(buttonClone, settings.sysColor, xpos,
                              settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
        xpos += (SMALL_BUTTON_SIZE() + DEFAULT_MARGIN());
        al_draw_tinted_bitmap(buttonLooppoint, settings.sysColor, xpos,
                              settings.tabArea.bottomRight.y + DEFAULT_MARGIN(), 0);
    } else if (status.currentTab > 1) {
        al_draw_text(font, settings.sysColor, referencePanel.printArea.topLeft.x, referencePanel.printArea.topLeft.y,
                     ALLEGRO_ALIGN_LEFT,
                     currentTune.config.blockTypes[status.get_current_blocktype()].blockConfigID.data());
...
void Main_Window::print_reference_data() {
    ...
    // TODO(utz): printing sequence should depend on blocktype config, not status == 0
    if (status.get_current_blocktype() == 0) {
        ...
        for (size_t pos = status.get_visible_first_reference_row(); pos < currentTune.sequence.size(); pos++) {
            ...
            al_draw_filled_rectangle(settings.referenceDataArea.topLeft.x, ypos,
                                     settings.referenceDataArea.topLeft.x + (16 * BT_CHAR_WIDTH()),
            ...
            string posstr = numToStr(static_cast<int64_t>(pos), 3, settings.hexMode);
            al_draw_text(font, color, settings.referenceDataArea.topLeft.x + 3 * BT_CHAR_WIDTH(),
                         ypos, ALLEGRO_ALIGN_RIGHT, posstr.data());

            if (pos == currentTune.sequenceLoopPoint) {
                al_draw_text(font, color, settings.referenceDataArea.topLeft.x + 3 * BT_CHAR_WIDTH(),
                             ypos, ALLEGRO_ALIGN_LEFT, "~");
            }

            al_draw_text(font, color, settings.referenceDataArea.topLeft.x + 4 * BT_CHAR_WIDTH(), ypos,
                         ALLEGRO_ALIGN_LEFT, currentTune.sequence[pos].data());
            ypos += CHAR_HEIGHT();
        ...
        if (status.vScrollbarRefSize < 1.0f) {
            ...
            status.vScrollbarRefSize *= (settings.referencePanelArea.bottomRight.y
                                         - settings.referencePanelArea.topLeft.y - (2* SMALL_BUTTON_SIZE()));

            status.vScrollbarRefStart = static_cast<float>(status.get_visible_first_reference_row())
                                        * ((settings.blockPanelArea.bottomRight.y - settings.blockPanelArea.topLeft.y
                                            - (2* SMALL_BUTTON_SIZE()))
                                           / static_cast<float>(currentTune.sequence.size()));
            ...
            status.vScrollbarRefSize = 0.0f;
        }
    } else {
        // print reference list
        for (size_t pos = status.get_visible_first_reference_row();
                pos < currentTune.blockTypes[status.get_current_blocktype()].blocks.size(); pos++) {
            al_draw_filled_rectangle(settings.referenceDataArea.topLeft.x, ypos,
                                     settings.referenceDataArea.topLeft.x + (12 * BT_CHAR_WIDTH()),
                                     ypos + CHAR_HEIGHT(),
                                     (((!status.focusBlock) && (pos == status.get_current_reference_row()))
                                      ? settings.cursorColor : settings.bgColor));

            ALLEGRO_COLOR color = (currentTune.blockTypes[status.get_current_blocktype()].blocks[pos].name
                                   == status.get_current_block_pointer()->name)
                                  ? settings.rowHlColor : settings.rowColor;
...
void Main_Window::print_block_data() {
    ...
    if (status.hScrollbarBlkSize < 1.0f) {
        ...
        al_draw_filled_rectangle(status.hScrollbarBlkStart, settings.blockPanelArea.bottomRight.y - SCROLL_BAR_SIZE(),
                                 status.hScrollbarBlkStart + status.hScrollbarBlkSize,
                                 settings.blockPanelArea.bottomRight.y - THIN_LINE(), settings.sysColor);
    } else {
        status.hScrollbarBlkSize = 0.0f;
    }
...
void Main_Window::print_row(const unsigned &row) {
    ...
    al_draw_text(font,
                 ((row == status.get_current_cursor_row()) && status.focusBlock) ? settings.rowActColor : txtColor,
                 settings.rowNumberArea.topLeft.x + 3 * BT_CHAR_WIDTH(),
                 settings.blockDataArea.topLeft.y
                 + (static_cast<float>(row - status.get_visible_first_row()) * CHAR_HEIGHT()), ALLEGRO_ALIGN_RIGHT,
                 numToStr(static_cast<int64_t>(row), 3, settings.hexMode).data());
...
void Main_Window::print_field(const unsigned &col, const unsigned &row) {
    ...
    float ypos = settings.blockDataArea.topLeft.y
                 + (static_cast<float>(row - status.get_visible_first_row()) * CHAR_HEIGHT());


    ALLEGRO_COLOR bgColor = (status.focusBlock && status.is_selected(row, col))
                            ? settings.selectionColor : settings.bgColor;
    ...
    ALLEGRO_COLOR txtColor = (row % status.rowHighlight) ? settings.rowColor : settings.rowHlColor;
    al_draw_text(font, (status.focusBlock && (row == status.get_current_cursor_row()))
                 ? settings.rowActColor : txtColor, xpos, ypos, ALLEGRO_ALIGN_LEFT,
...
void Main_Window::print_global_field(const unsigned &row) {
    ...
    al_draw_filled_rectangle(xpos, ypos, settings.blockDataArea.bottomRight.x, ypos + CHAR_HEIGHT(),
                             ((status.focusBlock &&  (row == status.currentGlobal))
                              ? settings.cursorColor : settings.bgColor));

    al_draw_text(font, (status.focusBlock && row == status.currentGlobal)
                 ? settings.rowActColor : settings.rowColor, xpos, ypos, ALLEGRO_ALIGN_LEFT,
                 currentTune.globalConstants[row].printString.data());
}


void Main_Window::print_dropdown() {
    // TODO(utz): limit list output if ypos out of blockDataArea bounds
    if (status.focusBlock) {
        if (status.currentTab > 0) {
            ...
            if (status.get_current_cursor_row() - status.get_visible_first_row() >= status.visibleRowsMax / 2) {
                // print upwards
                // TODO(utz): make window size dependant on command type, see downwards print
                ypos -= static_cast<float>(dropdown.options.size() * CHAR_HEIGHT());

                al_draw_filled_rectangle(xpos, (ypos >= settings.blockDataArea.topLeft.y)
                                         ? ypos : settings.blockDataArea.topLeft.y,
                                         xpos + ((currentField->command->mdCmdType == MD_WORD)
                                                 ? (9.0 * BT_CHAR_WIDTH()) : (5.0 * BT_CHAR_WIDTH())),
                                         (ypos + static_cast<float>((dropdown.options.size() + 1) * CHAR_HEIGHT())),
                                         settings.sysColor);
                al_draw_filled_rectangle(xpos + 1.0f, (ypos >= settings.blockDataArea.topLeft.y)
                                         ? ypos + 1.0f : settings.blockDataArea.topLeft.y + 1.0f,
                                         xpos + ((currentField->command->mdCmdType == MD_WORD)
                                                 ? (9.0 * BT_CHAR_WIDTH()) : (5.0 * BT_CHAR_WIDTH())) - 1.0f,
                                         (ypos + static_cast<float>((dropdown.options.size() + 1)
                                                 * CHAR_HEIGHT())) - 1.0f, settings.bgColor);
                ...
                al_draw_filled_rectangle(xpos, ypos, xpos + ((currentField->command->mdCmdType == MD_WORD)
                                         ? (9.0 * BT_CHAR_WIDTH()) : (5.0 * BT_CHAR_WIDTH())),
                                         ((ypos + static_cast<float>((dropdown.options.size() + 1)
                                                 * CHAR_HEIGHT()) < settings.blockDataArea.bottomRight.y)
                                          ? (ypos + static_cast<float>((dropdown.options.size() + 1)
                                                  * CHAR_HEIGHT())) : settings.blockDataArea.bottomRight.y),
                                         settings.sysColor);
                al_draw_filled_rectangle(xpos + 1.0f, ypos + 1.0f, xpos - 1.0f
                                         + ((currentField->command->mdCmdType == MD_WORD)
                                            ? (9.0 * BT_CHAR_WIDTH()) : (5.0 * BT_CHAR_WIDTH())),
                                         (ypos + static_cast<float>((dropdown.options.size() + 1)
                                                 * CHAR_HEIGHT()) < settings.blockDataArea.bottomRight.y)
                                         ? (ypos + static_cast<float>((dropdown.options.size() + 1)
            ...
            // TODO(utz): options list for global input
        }
    } else {
        ...
        if (status.get_current_reference_row()
                - status.get_visible_first_reference_row() >= status.visibleReferenceRowsMax / 2) {
            // print upwards
            ypos -= static_cast<float>(dropdown.options.size() * CHAR_HEIGHT());

            al_draw_filled_rectangle(xpos, (ypos >= settings.referenceDataArea.topLeft.y)
                                     ? ypos : settings.referenceDataArea.topLeft.y,
                                     xpos + (12.0 * BT_CHAR_WIDTH()),
                                     (ypos + static_cast<float>((dropdown.options.size() + 1) * CHAR_HEIGHT())),
                                     settings.sysColor);
            ...
            al_draw_filled_rectangle(xpos, ypos, xpos + (12.0 * BT_CHAR_WIDTH()),
                                     ((ypos + static_cast<float>((dropdown.options.size() + 1)
                                             * CHAR_HEIGHT()) < settings.referenceDataArea.bottomRight.y)
                                      ? (ypos + static_cast<float>((dropdown.options.size() + 1)
                                              * CHAR_HEIGHT())) : settings.referenceDataArea.bottomRight.y),
                                     settings.sysColor);
...
void Main_Window::set_display_title() {
    ...
    if (currentTune.savefilePath.empty()) {
        trackname += "unnamed";
    } else if (currentTune.savefilePath.size() < 237) {
        trackname += currentTune.savefilePath;
    } else {
        trackname += ("..." + currentTune.savefilePath.substr(currentTune.savefilePath.size() - 237, 237));
    }
    title += trackname;
...
void Main_Window::shutdown() {
    ...
    // temporary work-around because Windows version of liballeg ignores custom buttons,
    ...
    int doWhat = al_show_native_message_box(nullptr, "Save unsaved changes?", "",
                                            "The current tune has unsaved changes. Save changes before exiting?",
                                            nullptr, ALLEGRO_MESSAGEBOX_QUESTION|ALLEGRO_MESSAGEBOX_YES_NO);
...
bool Main_Window::display_msg_confirm_out_of_range_transpose(const unsigned &outOfRangeCount) {
    ...
    // temporary fix to work around keyboard_state desync bug in Allegro5
    al_uninstall_keyboard();
    al_install_keyboard();
    ...
    for (unsigned i = 0; i < currentTune.config.blockTypes.size(); i++) {
        ...
        for (auto&& it : currentTune.blockTypes[i].commands) {
            ...
            if (it->defaultSubstitute) {
                ...
                al_append_native_text_log(engineInfo, "%s", it->mdCmdDefaultValString.data());
            }
            al_append_native_text_log(engineInfo, "%s", "\n");
            al_append_native_text_log(engineInfo, "%s", "Flags: ");
            ...
            if (it->isBlkReference) {
                al_append_native_text_log(engineInfo, "%s", "reference_cmd ");
            } else if (it->mdCmdForceString) {
                al_append_native_text_log(engineInfo, "%s", "text_cmd ");
            }
