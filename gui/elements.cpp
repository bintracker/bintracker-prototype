...
void Panel::create(const Area &total_area, const int &border_mask, const int &scrollbar_mask,
    ...
    switch (border_mask) {
        case DIR_TOP:
            al_draw_line(totalArea.topLeft.x, totalArea.topLeft.y, totalArea.bottomRight.x, totalArea.topLeft.y,
                         sysColor, THICK_LINE());
        ...
        case DIR_LEFT:
            al_draw_line(totalArea.topLeft.x, totalArea.topLeft.y, totalArea.topLeft.x, totalArea.bottomRight.y,
                         sysColor, THICK_LINE());
            printArea.topLeft.x += THICK_LINE();
            break;

        case DIR_RIGHT:
            al_draw_line(totalArea.bottomRight.x - THICK_LINE_OFFSET(), totalArea.topLeft.y,
                         totalArea.bottomRight.x - THICK_LINE_OFFSET(), totalArea.bottomRight.y, sysColor,
                         THICK_LINE());
    ...
    if (scrollbar_mask & DIR_RIGHT) {
        ...
        al_draw_tinted_bitmap(win->arrowUp, sysColor, totalArea.bottomRight.x - SCROLL_PANE_SIZE(),
                              totalArea.topLeft.y, 0);
    ...
    if (scrollbar_mask & DIR_BOTTOM) {
        ...
        if (verticalScroll) {
            al_draw_line(totalArea.topLeft.x, totalArea.bottomRight.y - SCROLL_BAR_SIZE(),
                         totalArea.bottomRight.x - SCROLL_BAR_SIZE(),
                         totalArea.bottomRight.y - SCROLL_BAR_SIZE(), sysColor, THIN_LINE());
            al_draw_tinted_bitmap(win->arrowLeft, sysColor, totalArea.topLeft.x,
                                  totalArea.bottomRight.y - SCROLL_PANE_SIZE(), 0);
...
Global_Settings::Global_Settings(): kbdLang("EN"), defaultConfig("BetaPhase") {
    ...
    for (auto&& it : ini_lines) {
        ...
        if (it.find("XRES=") != string::npos) {
            ...
            if (!isNumber(tempstr)) {
                cout << "Error: Invalid XRES argument in settings.ini." << endl;
            } else {
                xres = static_cast<int>(strToNum(tempstr));
            }
            if (xres < 720) xres = 720;
        } else if (it.find("YRES=") != string::npos) {
            tempstr = it.substr(5, string::npos);
            if (!isNumber(tempstr)) {
                cout << "Error: Invalid YRES argument in settings.ini." << endl;
            } else {
                yres = static_cast<int>(strToNum(tempstr));
            }
            ...
            tempstr = it.substr(8, string::npos);
            if (!isNumber(tempstr)) {
                cout << "Error: Invalid BGCOLOR argument in settings.ini." << endl;
            } else {
                int64_t rgb = strToNum(tempstr);
            ...
            tempstr = it.substr(9, string::npos);
            if (!isNumber(tempstr)) {
                cout << "Error: Invalid ROWCOLOR argument in settings.ini." << endl;
            } else {
                int64_t rgb = strToNum(tempstr);
            ...
            if (!isNumber(tempstr)) {
                cout << "Error: Invalid ROWHIGHLIGHTCOLOR argument in settings.ini." << endl;
            } else {
                int64_t rgb = strToNum(tempstr);
            ...
            if (!isNumber(tempstr)) {
                cout << "Error: Invalid ROWACTIVECOLOR argument in settings.ini." << endl;
            } else {
                int64_t rgb = strToNum(tempstr);
            ...
            if (!isNumber(tempstr)) {
                cout << "Error: Invalid CURSORCOLOR argument in settings.ini." << endl;
            } else {
                int64_t rgb = strToNum(tempstr);
            ...
            tempstr = it.substr(15, string::npos);
            if (!isNumber(tempstr)) {
                cout << "Error: Invalid SELECTIONCOLOR argument in settings.ini." << endl;
            } else {
                int64_t rgb = strToNum(tempstr);
            ...
            if (!isNumber(tempstr)) {
                cout << "Error: Invalid DEFAULTROWHIGHLIGHT argument in settings.ini." << endl;
            } else {
                defaultRowHighlight = static_cast<unsigned>(strToNum(tempstr));
            }
        } else if (it.find("DEFAULTBLOCKLENGTH=") != string::npos) {
            tempstr = it.substr(19, string::npos);
            if (!isNumber(tempstr)) {
                cout << "Error: Invalid DEFAULTBLOCKLENGTH argument in settings.ini." << endl;
            } else {
                defaultBlockLength = static_cast<unsigned>(strToNum(tempstr));
            }
            ...
            if (!isNumber(tempstr)) {
                cout << "Error: Invalid CHUNKSIZE argument in settings.ini." << endl;
            } else {
                audioChunkSize = static_cast<unsigned>(strToNum(tempstr));
            }
            ...
            tempstr = it.substr(15, string::npos);
            if (!isNumber(tempstr)) {
                cout << "Error: Invalid KEYREPEATDELAY argument in settings.ini." << endl;
            } else {
                keyRepeatDelay = static_cast<unsigned>(strToNum(tempstr));
            }
        } else if (it.find("SIMPLEGFXBUFFER=false") != string::npos) {
            simpleGfxBuffer = false;
        }
        // TODO(utz): DEC numberbase is unstable, disable it for now
    ...
    while (getline(CONFIGLIST, tempstr)) {
        if (!tempstr.empty()) configList.push_back(tempstr);    // TODO(utz): validate entries,
    }                                                           // ie. check if configs are present
    CONFIGLIST.close();


    buttonPanelArea.set(Point(0.0f, 0.0f), Point(xres, BUTTON_PANEL_HEIGHT()));
    referencePanelArea.set(Point(xres - REFERENCE_PANEL_WIDTH(), buttonPanelArea.bottomRight.y),
                           Point(xres, yres - MESSAGE_PANEL_HEIGHT()));
    blockPanelArea.set(Point(0.0f, BUTTON_PANEL_HEIGHT()), Point(xres - REFERENCE_PANEL_WIDTH(),
                       yres - MESSAGE_PANEL_HEIGHT()));
    messagePanelArea.set(Point(0.0f, yres - MESSAGE_PANEL_HEIGHT()), Point(xres, yres));

    // TODO(utz): add THICK_LINE offsets
    ...
    blockHeaderArea.set(blockArea.topLeft, Point(blockArea.bottomRight.x,
                        blockArea.topLeft.y + CHAR_HEIGHT() + DEFAULT_MARGIN()));

    blockNameArea.set(Point(blockPanelArea.topLeft.x + SMALL_BUTTON_SIZE() + (2 * DEFAULT_MARGIN()),
                            tabArea.bottomRight.y + DEFAULT_MARGIN()),
                      Point(blockPanelArea.topLeft.x + SMALL_BUTTON_SIZE() +
                            (2 * DEFAULT_MARGIN()) + (12 * BT_CHAR_WIDTH()),
                            tabArea.bottomRight.y + (2 * DEFAULT_MARGIN()) + CHAR_HEIGHT()));

    blockLengthArea.set(Point(blockPanelArea.topLeft.x +
                              (2 * SMALL_BUTTON_SIZE()) + (19 * BT_CHAR_WIDTH()) + (5 * DEFAULT_MARGIN()),
                              tabArea.bottomRight.y + 2.0f + DEFAULT_MARGIN()),
                        Point(blockPanelArea.topLeft.x + (2 * SMALL_BUTTON_SIZE()) + (22 * BT_CHAR_WIDTH()) +
                              (5 * DEFAULT_MARGIN()),
                              tabArea.bottomRight.y + 2.0f + DEFAULT_MARGIN() + CHAR_HEIGHT()));

    rowNumberArea.set(Point(blockPanelArea.topLeft.x + DEFAULT_MARGIN(),
                            blockHeaderArea.bottomRight.y + DEFAULT_MARGIN()),
                      Point(blockPanelArea.topLeft.x + ROW_COLUMN_WIDTH() + DEFAULT_MARGIN(),
                            blockPanelArea.bottomRight.y - DEFAULT_MARGIN()));

    columnHeaderArea.set(Point(rowNumberArea.bottomRight.x + DEFAULT_MARGIN(),
                               blockHeaderArea.bottomRight.y + DEFAULT_MARGIN()),
                         Point(blockPanelArea.bottomRight.x - DEFAULT_MARGIN() - SCROLL_PANE_SIZE(),
                               blockHeaderArea.bottomRight.y + CHAR_HEIGHT() + DEFAULT_MARGIN()));

    blockDataArea.set(Point(blockPanelArea.topLeft.x + DEFAULT_MARGIN(),
                            columnHeaderArea.bottomRight.y + DEFAULT_MARGIN()),
                      Point(blockPanelArea.bottomRight.x - DEFAULT_MARGIN() - SCROLL_PANE_SIZE(),
                            blockPanelArea.bottomRight.y - DEFAULT_MARGIN() - SCROLL_PANE_SIZE()));
