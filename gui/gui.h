...
constexpr const float THICK_LINE() {
    return 2.0f;
}
constexpr const float THICK_LINE_OFFSET() {
    return 1.0f;
}
constexpr const float THIN_LINE() {
    return 1.0f;
}
constexpr const float DEFAULT_MARGIN() {
    return 4.0f;
}
constexpr const float BUTTON_PANEL_HEIGHT() {
    return 40.0f;
}
constexpr const float REFERENCE_PANEL_WIDTH() {
    return 204.0f;
}
constexpr const float MESSAGE_PANEL_HEIGHT() {
    return 24.0f;
}
constexpr const float CHAR_HEIGHT() {
    return 14.0f;
}
constexpr const float BT_CHAR_WIDTH() {
    return 10.0f;
}
constexpr const float BUTTON_SIZE() {
    return 28.0f;
}
constexpr const float SMALL_BUTTON_SIZE() {
    return 16.0f;
}
constexpr const float SCROLL_PANE_SIZE() {
    return 16.0f;
}
constexpr const float SCROLL_BAR_SIZE() {
    return 15.0f;
}
constexpr const float ROW_COLUMN_WIDTH() {
    return 34.0f;
}
constexpr const float DEFAULT_FPS() {
    return (1.0f / 22.5f);
}
...
class Point {
 public:
    ...
    explicit Point(const float &xpos = 0.0f, const float &ypos = 0.0f);
...
class Block_Tab {
 public:
    ...
    explicit Block_Tab(const float &_xpos = 0.0f, std::string title_ = "");
...
class Display_Status {
 public:
    // TODO(utz): should Display_Status handle auto_inc?
    // TODO(utz): make stuff private
    unsigned currentTab;
    ...
    std::vector<unsigned> visibleFirstColumns;
    std::vector<unsigned> visibleLastColumns;
    std::vector<unsigned> visibleFirstRows;
    std::vector<unsigned> currentBlocks;
    std::vector<unsigned> currentCsrColumns;
    std::vector<unsigned> currentCsrRows;
    std::vector<unsigned> visibleFirstReferenceRows;
    std::vector<unsigned> currentReferenceRows;
    ...
    unsigned get_reference_iterator(const std::string &blockID);    // TODO(utz): temp made public, make private again
...
class Changelog_Entry {
 public:
    explicit Changelog_Entry(Work_Tune *tune = nullptr, Display_Status *status = nullptr);
 ...
 private:
    ...
    std::vector<Datablock_Field> lastUsed;      // contains values set before the current line
    std::vector<Datablock_Field> globalConstants;
    std::vector<std::string> sequence;
    std::vector<Datablock_Type> blockTypes;
...
class Options_Dropdown {
 public:
    ...
    void activate(const std::vector<std::string> &_options, const std::string &_userEntry);
    void collapse();
    void select_next();
    void select_previous();
    std::string get_current_option();
};
...
class Main_Window {
 public:
    ...
    ALLEGRO_EVENT_QUEUE *eventQueue;    // TODO(utz): consider several event queues for mouse, kbd, other
    ...
    explicit Main_Window(const std::string &initFile = "");
    ~Main_Window();
 ...
 private:
    ...
    std::vector<ALLEGRO_BITMAP*> menuButtons;
    ...
    std::vector<Block_Tab> tabPanel;        // TODO(utz): legacy code

    Datablock_Field *currentField;

    std::string userInputString;

    std::vector<Changelog_Entry> undoStack;
    std::vector<Changelog_Entry> redoStack;
    ...
    unsigned get_reference_iterator(const std::string &blockID);
    void make_new_file(const std::string &config);
    void open_file();
    void save_file();
    void save_file_as();
    ...
    std::string receive_string_input();
    void erase_data_input();
    void complete_data_input();
    void cancel_data_input();
    std::string auto_complete(const std::vector<std::string> &options);
