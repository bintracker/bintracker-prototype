...
void Sound_Emul::init(Work_Tune *tune, const unsigned &audioChunkSize_) {
	...
	if (!al_attach_audio_stream_to_mixer(audioStream, al_get_default_mixer())) throw (string("Failed to attach audio stream to mixer."));


//	setup_vm(machine, engineCode_, engineCodeNoLoop_, engineSize_, engineSizeNoLoop);
    setup_vm();

	stop();
}

//void Sound_Emul::setup_vm(const string &machine, char *engineCode_, char *engineCodeNoLoop_,
//    const int &engineSize_, const int &engineSizeNoLoop_) {
void Sound_Emul::setup_vm() {
	...
	else throw (string("Unknown machine configuration, failed to initialize sound emulation."));

//	engineCode = engineCode_;
//	engineCodeNoLoop = engineCodeNoLoop_;
//	engineSize = engineSize_;
//	engineSizeNoLoop = engineSizeNoLoop_;
}
...
//	cout << "player started" << endl;
	...
	//ignore request for player start if trying to play a row and player is already playing pattern/song
//	cout << "curr. playmode: " << playMode << ", requested: " << playMode_ << endl;
...
//	WAVEFILE << "WAVEfmt \x10" << nil << nil << nil << "\x1" << nil << "\x1" << nil << "\x44\xac" << nil << nil << "\x88\x58\x1" << nil;
//	filesize -= 36;
